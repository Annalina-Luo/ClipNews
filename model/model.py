...
class EncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask, imgs):
        ...
        src2_ = self.tanh(
            self.l2(src2.expand(src2.size(0), src.size(1), src2.size(2))))
...
class Decoder(nn.Module):
    ...
    def forward(self, trg, enc_src, src, trg_mask, src_mask, imgs, enc_ref, reference, ref_mask):
        ...
        for layer in self.layers:
            trg, trg_a, trg_v, trg_r, attention, attention2 = layer(
                trg, enc_src, trg_mask, src_mask, imgs, enc_ref, ref_mask)
        ...
        index1 = index1.expand(attention.size(1), index1.size(
        ...
        index2 = index2.expand(attention2.size(1), index2.size(
            0), index2.size(1)).permute(1, 0, 2)
        attn_value1 = torch.zeros(
            [output.size(0), output.size(1), output.size(2)]).to(device)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask, imgs, enc_ref, ref_mask):
        ...
        # encoder attention
        _trg0, attention = self.encoder_attention(
            trg, enc_src, enc_src, src_mask)
        _trg2, attention2 = self.encoder_attention2(
            trg, enc_ref, enc_ref, ref_mask)
        ...
        trg = self.enc_attn_layer_norm(
            trg + self.dropout(_trg0) + self.dropout(_trg1) + self.dropout(_trg2))
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, reference, trg, imgs):
        ...
        output, attention = self.decoder(
            trg, enc_src, src, trg_mask, src_mask, imgs, enc_ref, reference, ref_mask)
    ...
    for i in range(max_length):
        ...
        with torch.no_grad():
            output, attention = model.decoder(
                trg_tensor, enc_src, src, trg_mask, src_mask, imgs, enc_ref, reference, ref_mask)
